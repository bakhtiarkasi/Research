...
class Commit
  ...
  def starBuildTest()
  	Dir.chdir($workDir) do
	   ...
	   puts "\nChanging build file";
           %x[sed -i.bak 's|http://maven.glassfish.org/content/groups/public/|http://repo.jenkins-ci.org/public/|g' pom.xml]

	   puts "\nCopying license file";
	   %x[cp -f -r ../licenseCompleter.groovy ./]

	   puts "\nMaking builder";
	   #puts %x[/usr/share/java/maven-2.2.0/bin/mvn -Plight-test -e -fae clean package install
	    puts %x[ mvn -Plight-test -e -fae clean package install  ]
...
#Create a read only object of the file
mergeConfs = File.new("jenkins1.ids", "r")
