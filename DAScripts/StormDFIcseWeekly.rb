...
class ProcessSummary
	...
	def processHash(fromDate, toDate)
		Dir.chdir($workDir) do
			filesInfo = %x[#{$gremlinPath} -e StormICSEWeekly.groovy #{fromDate} #{toDate}];
			...
			for ss in 2...masterInfo.count()
				$xmlstring += "<File FileName=\"#{masterInfo[ss].gsub("storm--src/jvm", "classes").gsub("storm--src/clj", "classes")}\">";
			...
			for merge in filesInfo[1..filesInfo.size()]
				...
				for ss in 2...remoteInfo.count()
					$xmlstring += "<File FileName=\"#{remoteInfo[ss].gsub("storm--src/jvm", "classes").gsub("storm--src/clj", "classes")}\">";
...
if allFiles.nil?
	mergeConfs = File.new("StormWeekly.ids", "r")

	mergeConfs.each do |hashCode|
		hashCode = hashCode.chomp;
		hashId = hashCode.split(":");
		$hashCode = hashId[0]	
		d =  DateTime.parse(hashId[1]);
		startDate = d - (d.to_date.cwday % 7);
		endDate =  d + 7 - (d.to_date.cwday % 7) - 1;

		
	gitSummary.processHash(startDate.to_s.split("T")[0], endDate.to_s.split("T")[0]);
end
