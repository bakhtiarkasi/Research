...
$workDir = "/Users/bkasi/Documents/Research/DataAnalysis/Storm";
$sortedList = Hash.new;
...
class Commit
  ...
  def starBuildTest()
  	Dir.chdir($workDir) do
	    ...
	    puts"\n\n Building Code"
            #puts %x[mvn -f pom.xml clean package]
            puts %x[lein jar]
            puts"\n\n";

        puts"\nCopying class files to output folder";
        %x[mkdir -p ../StormRelease/#{parentHash}]
        %x[cp -R classes ../StormRelease/#{parentHash}]
        %x[cp *.jar ../StormRelease/#{parentHash}]
		
		puts "--------------------------------------------------------------------------------"

		puts"Removing integration tests";
		%x[rm -r -f ./test/clj/backtype/storm/integration_test.clj ]
		%x[rm -r -f ./test/clj/backtype/storm/local_state_test.clj ]
		%x[rm -r -f ./test/clj/backtype/storm/multilang_test.clj ]		
...
puts "\nGetting Git repository for Storm";
#%x[git clone git://perl5.git.perl.org/perl.git #{$workDir}]

#Create a read only object of the file
mergeConfs = File.new("storm1.ids", "r")
