...
public class SimulatorTest {
	...
	private final static Logger log4j = Logger.getLogger(SimulatorTest.class
			.getName());
	...
	public void testAllTaskAssigned() {
		...
		try {
			...
			for (SimulatorSet st : dataTestSet) {
			    ...
			    log4j.info("Check if all tasks are assigned");
				...
				for (Integer key : tasksDataSet.keySet()) {
					...
					// conflicts
					log4j.info("Check if a task is still marked clean when it has conflicts");
					...
					// conflicts
					log4j.info("Check if all conflicts counts different than individual");
				...
				// conflicts
				log4j.info("check if total conflicts are same as individual task conflicts");
				...
				log4j.info("check if merge conflict percentage is same as provided");
				
				
				log4j.info("MC Files after " + totalMergeConflicts + " all tasks " + obj.getAllPossibleConflicts() );
				log4j.info( ""+Math
				...
				log4j.info("check if build conflict percentage is same as provided");
				log4j.info("BC Files after " + totalBuildConflicts + " all tasks " + obj.getAllPossibleConflicts() );
				...
				log4j.info("check if test conflict percentage is same as provided");
				log4j.info("TC Files after " + totalTestConflicts + " all tasks " + obj.getAllPossibleConflicts() );
				...
				log4j.info("SP Params: "+ sessionId+ "," +allTasksCount+ "," +allFilesCount+ "," +st.noOfDevelopers+ "," + totalMergeConflicts+ "," +totalBuildConflicts+ "," +totalTestConflicts+ "," +totalConflicts/2);
