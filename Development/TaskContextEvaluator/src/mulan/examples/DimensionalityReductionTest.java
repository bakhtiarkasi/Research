...
public class DimensionalityReductionTest {
    ...
    public static void main(String[] args) throws Exception {
       ...
       //-> ASEvaluation ase = new GainRatioAttributeEval();
        ...
        BinaryRelevanceAttributeEvaluator ae = new 
        BinaryRelevanceAttributeEvaluator(ase, mlData, "max", "none", "eval");
        ...
        for(int i= 0; i < mlData.getFeatureIndices().length; i++)
        {
        	...
        	if(ae.evaluateAttribute(mlData.getDataSet().attribute(mlData.getFeatureIndices()[i]).index()) > 10.83)
        		count++;
        }
        System.out.println("\n\n\n" +  count + "\\" + mlData.getFeatureIndices().length + "\n\n");

        final int NUM_TO_KEEP = 500;
